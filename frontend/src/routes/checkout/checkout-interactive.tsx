...
export function CheckoutInteractive() {
  ...
  const search = useSearch({ from: '/checkout' }) as Record<string, string | undefined>
  ...
  const [checkoutType, setCheckoutType] = useState<'subscription' | 'order' | null>(null)
  ...
  useEffect(() => {
    ...
    if (productIdParam || orderTypeParam === 'order' || (itemParam && priceParam)) {
      ...
      if (productIdParam) {
        ...
        if (product) {
          ...
          const quantity = !Number.isNaN(parsedQuantity) && parsedQuantity > 0 ? parsedQuantity : 1
          computedOrderItems = [
            {
              name: product.name,
          ...
          <AlertDescription className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                {t('common_try_calcpro_risk_free_for_14_days_no_credit_card_required_to')}
              </p>
            </div>
