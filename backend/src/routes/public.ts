...
publicRoutes.post(
  ...
  async (c) => {
    try {
      ...
      if (isSuspicious) {
        return c.json({ error: "Your message could not be submitted. Please ensure your message contains appropriate content." }, 400);
      }
      ...
      return c.json({ error: "Unable to send your message at this time. Please try again later." }, 500);
    }
  }
);
...
publicRoutes.post(
  ...
  async (c) => {
    try {
      ...
      if (result.success) {
        return c.json({ success: true, message: "Confirmation email sent successfully", emailId: result.id });
      } else {
        return c.json({ error: "Failed to send confirmation email", details: result.error }, 500);
      }
    } catch (error) {
