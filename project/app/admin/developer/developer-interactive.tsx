/**
 * Developer Interactive Component - Client Component
 *
 * Handles interactive parts of developer page: database tools, JSON editing, and table visualization
 */
...
export function DeveloperInteractive() {
  ...
  const fetchTableData = useCallback(
    async (page: number = 1) => {
      ...
      try {
        ...
        let message = t("admin_developer_error_unknown");
        ...
        setError(`${t("admin_developer_error_fetch_table")}: ${message}`);
        setTableData([]);
      } finally {
        setLoading(false);
      }
  ...
  // Memoize table selection handler
  const handleTableChange = useCallback(
    (value: string) => {
      const table = tables.find((t) => t.name === value) || null;
  ...
  // Memoize expected params display
