 ...
 * Support Center Page
 *
 * Getting started guide, account & billing FAQ, and troubleshooting steps.
 */
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return createPageMetadata({
    ...
    description: t("metadata_support_description"),
    path: "/support",
  }).metadata;
}
          ...
          <Card className="border-2 shadow-lg">
            ...
            <CardContent className="space-y-3">
              {[
                ...
                { name: "Investment Calculator", path: "/calculator" },
                { name: "Retirement Calculator", path: "/calculator" },
              ].map(({ name, path }) => (
          ...
          <Card className="border-2 shadow-lg">
            <CardHeader>
              <CardTitle className="text-2xl flex items-center gap-2">
                ...
                {t("support_troubleshooting_title")}
              </CardTitle>
            </CardHeader>
            <CardContent>
              <Accordion type="single" collapsible className="w-full">
                {troubleshootingFAQs.map((item, i) => (
                  <AccordionItem
                    key={i}
                    value={`troubleshoot-${i}`}
                    className="border-b last:border-b-0"
                  >
                    <AccordionTrigger className="text-left font-semibold hover:no-underline py-4">
                      {item.question}
